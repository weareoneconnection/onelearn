  ...
  privacy: {
    zh: {
      ...
      sections: [
        ...
        { heading: "3. 委托处理与第三方", body: ["为提供服务，我们会将必要数据交由以下服务商处理：OpenAI（AI 生成、资料检索，以及语音导师的实时语音处理；我们不录音、不保存语音）、Clerk（登录认证）、Stripe（支付）、Turso / Cloudflare（数据存储）、Vercel（网站托管）、Resend（复习提醒邮件发送）、PostHog（产品使用分析，仅使用经哈希处理的标识，不记录邮箱）、Sentry（错误监控，如启用）。", "上述服务商可能位于中国境外。使用本服务即表示你知悉并同意为提供服务所必需的个人信息出境；我们将按照适用法律（包括马来西亚《2010 年个人数据保护法》以及你所在地的个人信息保护法律）履行相应义务，并与服务商约定数据保护要求。"] },
