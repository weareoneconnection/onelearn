  ...
  privacy: {
    zh: {
      ...
      sections: [
        ...
        { heading: "3. 委托处理与第三方", body: ["为提供服务，我们会将必要数据交由以下服务商处理：OpenAI（AI 生成与资料检索）、Clerk（登录认证）、Stripe（支付）、Turso / Cloudflare（数据存储）、Vercel（网站托管）、【错误监控服务，如 Sentry】。", "上述服务商可能位于中国境外。使用本服务即表示你知悉并同意为提供服务所必需的个人信息出境；我们将按照适用法律（包括《个人信息保护法》）的要求履行相应义务【如需完成个人信息出境安全评估、标准合同备案等，请在此说明】。"] },
    ...
    en: {
      ...
      sections: [
        ...
        { heading: "3. Processors", body: ["We share the data needed to run the Service with: OpenAI (AI generation and retrieval), Clerk (authentication), Stripe (payments), Turso / Cloudflare (storage), Vercel (hosting), and 【error monitoring, e.g. Sentry】.", "These providers may be located outside your country. We transfer personal information across borders only as needed to provide the Service and in line with applicable law 【describe the cross-border transfer mechanism】."] },
