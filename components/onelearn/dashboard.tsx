...
import { Activity, ArrowRight, BrainCircuit, ChevronRight, Database, FileCheck2, MoreHorizontal, Sparkles, TimerReset } from "lucide-react";
import Link from "next/link";
import { Button } from "@/components/ui/button";
import { Progress } from "@/components/ui/progress";
import { cn } from "@/lib/utils";
import { type Locale, pick } from "@/lib/onelearn/i18n";
import type { LearnerIdentity, LearnerStats, MasteryOverview, View } from "./types";
import { MetricCard } from "./ui";

export function Dashboard({ onNavigate, locale, identity, stats, storage, mastery }: { onNavigate: (view: View) => void; locale: Locale; identity: LearnerIdentity | null; stats: LearnerStats | null; storage: "durable" | "ephemeral"; mastery: MasteryOverview | null }) {
  ...
  const streak = mastery?.streak.current ?? 0;
  const weekAnswers = mastery?.week.answers ?? 0;
  const weekAccuracy = weekAnswers ? Math.round(((mastery?.week.correct ?? 0) / weekAnswers) * 100) : 0;
  return <div className="space-y-7 animate-in fade-in duration-500">
    <section className="dashboard-hero">
      <div className="relative z-10 max-w-2xl"><div className="eyebrow"><Sparkles className="size-3.5" />{storage === "durable" ? l("跨设备学习档案已同步", "CROSS-DEVICE PROFILE SYNCED") : l("设备模式 · 登录后跨设备同步", "DEVICE MODE · SIGN IN TO SYNC")}</div><h1>{l(`你好，${identity?.displayName ?? "学习者"}。`, `Hello, ${identity?.displayName ?? "learner"}.`)}</h1><p>{streak > 0
        ? l(`已连续学习 ${streak} 天${mastery?.streak.activeToday ? "，今天的学习已记录" : "，今天学一点就能保持"}。`, `${streak}-day streak${mastery?.streak.activeToday ? " — today counts already" : " — study a little today to keep it"}.`)
        : l("课程、资料、练习和 AI 导师证据现在进入同一份学习档案。", "Courses, sources, practice, and AI tutor evidence now flow into one learning profile.")}
        {weekAnswers > 0 && l(` 本周答题 ${weekAnswers} 道，正确率 ${weekAccuracy}%。`, ` This week: ${weekAnswers} answers, ${weekAccuracy}% correct.`)}</p><div className="mt-7 flex flex-wrap gap-3"><Button onClick={() => onNavigate("learn")} className="primary-pill">{l("继续学习", "Continue learning")} <ArrowRight /></Button><Button onClick={() => onNavigate("path")} variant="outline" className="secondary-pill">{l("查看知识地图", "View knowledge map")}</Button><Link href={locale === "zh" ? "/guide" : "/guide?lang=en"} className="self-center text-sm text-cyan-200 underline-offset-4 hover:underline">{l("新手？3 分钟上手指南 →", "New here? 3-minute guide →")}</Link></div></div>
