...
import { ArrowRight, Check, LoaderCircle, Mic, Orbit, Play, ShieldCheck, TimerReset, TriangleAlert } from "lucide-react";
...
export function LearningRoom({ locale, bundle }: { locale: Locale; bundle: GeneratedCourseBundle | null }) {
  const l = (zh: string, en: string) => pick(locale, zh, en);
...
export function QuestionCard({ locale, bundle, mode, onAnswered }: { locale: Locale; bundle: GeneratedCourseBundle | null; mode: "practice" | "review"; onAnswered: () => void }) {
  const l = (zh: string, en: string) => pick(locale, zh, en);
  const lesson = bundle?.curriculum.firstLesson ?? fallbackLesson(locale);
...
export function PracticeView({ locale, bundle, onAnswered }: { locale: Locale; bundle: GeneratedCourseBundle | null; onAnswered: () => void }) {
  const l = (zh: string, en: string) => pick(locale, zh, en);
...
export function ReviewView({ locale, bundle, mastery, onAnswered, onNavigate }: { locale: Locale; bundle: GeneratedCourseBundle | null; mastery: MasteryOverview | null; onAnswered: () => void; onNavigate: (view: View) => void }) {
  const l = (zh: string, en: string) => pick(locale, zh, en);
